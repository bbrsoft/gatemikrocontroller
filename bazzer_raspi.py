...
import requests
import os
import cv2  # OpenCV untuk menangkap stream RTSP
...
# Fungsi untuk menangkap snapshot dari kamera Imou
def ambil_gambar_dari_imou(rtsp_url, filename="snapshot.jpg"):
    ...
    if ret:
        ...
        print("📷 Snapshot berhasil diambil.")
    else:
...
try:
    while True:
        if GPIO.input(PIR_PIN):
            ...
            # Ambil gambar dari kamera Imou setelah deteksi gerakan
            rtsp_url = "rtsp://<username>:<password>@<ip_address>:554/cam/realmonitor?channel=1&subtype=0"  # Ganti dengan URL RTSP kamera Imou
            filename = "deteksi_imou.jpg"
            ambil_gambar_dari_imou(rtsp_url, filename)

            # Kirim ke Telegram
            kirim_telegram_pesan("🚨 Gerakan terdeteksi! Mengirim foto dari kamera Imou...")
